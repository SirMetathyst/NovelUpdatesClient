...
func TestDoSearchRequest_Parse_ID_Success(t *testing.T) {

	response, _ := DoSearchRequestWith(TestSuccessRequester, nil)

	assert.Equal(t, "another-world-awakening-transcendental-create-skill", response[0].ID)
	assert.Equal(t, "ill-become-a-villainess-that-will-go-down-in-history", response[1].ID)
	assert.Equal(t, "i-help-the-richest-man-spend-money-to-prevent-disasters", response[2].ID)
}

func TestDoSearchRequest_Parse_ID_Failure(t *testing.T) {

	response, _ := DoSearchRequestWith(TestFailureRequester, nil)

	assert.Equal(t, "ERR", response[0].ID)
	assert.Equal(t, "ERR", response[1].ID)
	assert.Equal(t, "ERR", response[2].ID)
}

func TestDoSearchRequest_Parse_URL_Success(t *testing.T) {

	response, _ := DoSearchRequestWith(TestSuccessRequester, nil)

	assert.Equal(t, "https://www.novelupdates.com/series/another-world-awakening-transcendental-create-skill/", response[0].URL)
	assert.Equal(t, "https://www.novelupdates.com/series/ill-become-a-villainess-that-will-go-down-in-history/", response[1].URL)
	assert.Equal(t, "https://www.novelupdates.com/series/i-help-the-richest-man-spend-money-to-prevent-disasters/", response[2].URL)
}

func TestDoSearchRequest_Parse_URL_Failure(t *testing.T) {

	response, _ := DoSearchRequestWith(TestFailureRequester, nil)

	assert.Equal(t, "ERR", response[0].URL)
	assert.Equal(t, "ERR", response[1].URL)
	assert.Equal(t, "ERR", response[2].URL)
}
